...
try:
    from backend.llm.first_launch import (
        check_and_trigger_first_launch,
        is_first_launch,
        is_auto_download_complete,
    )

except Exception as _e:
    logger.warning("初回起動モジュールのインポートに失敗しました: %s", _e)
...
/* ════════════════════════════════════════
   Interpretation Panel V2 - Premium Styling
   ════════════════════════════════════════ */

/* Glassmorphism cards for interpretation */
.interp-glass-card {
...
.interp-glass-card:hover {
    border-color: rgba(0, 212, 255, 0.3) !important;
    box-shadow: 0 8px 32px rgba(0, 212, 255, 0.1) !important;
}

/* SHAP tab accent */
.tab-shap .q-tab--active {
    color: #00d4ff !important;
    border-bottom-color: #00d4ff !important;
}

/* SAGE tab accent */
.tab-sage .q-tab--active {
    color: #4ade80 !important;
    border-bottom-color: #4ade80 !important;
}

/* SRI tab accent */
.tab-sri .q-tab--active {
    color: #a855f7 !important;
    border-bottom-color: #a855f7 !important;
}

/* Animated gradient border for premium badge */
@keyframes gradient-border {
    0% { border-color: #00d4ff; }
    33% { border-color: #7c4dff; }
    66% { border-color: #4ade80; }
...
def main_page():
    ...
    async def _run_analysis():
        ...
        run_btn.disable()
        run_btn.text = "⏳ 解析中..."
        run_btn.classes.remove("btn-run-analysis")
        try:
    ...
    ui.keyboard(
        on_key=lambda e: (
            _run_analysis() if e.key == 'Enter' and e.modifiers.ctrl and e.action != 'repeat' else
    ...
    with ui.left_drawer(value=True).classes("bg-dark q-pa-md").props("width=240"):
        ...
        ui.separator()
        ui.button(
            "📂 解析設定", on_click=lambda: main_tabs.set_value("data")
        ...
        ui.separator()
        ui.button(
            "🤖 LLMアドバイス", on_click=lambda: _show_llm_advice_dialog()
        ).props("flat color=white align=left size=sm no-caps").classes("full-width q-mb-xs")
    ...
    with ui.tabs().classes("full-width q-mt-xs").props(
        ...
        data_tab = ui.tab("data", label="📂 解析設定", icon="settings")
        ...
        llm_tab = ui.tab("llm", label="🤖 LLM", icon="smart_toy")
    ...
    with ui.tab_panels(main_tabs, value=data_tab).classes("full-width"):
        ...
        with ui.tab_panel(data_tab):
            ...
            # LLMクイックアクセス: データ読込後に分析ダイアログを起動
            ui.separator().classes("q-my-sm")
            with ui.row().classes("items-center justify-end q-mt-sm"):
                ui.button(
                    "🤖 LLMでデータ分析",
                    on_click=lambda: _trigger_llm_from_data_tab(),
                ).props("flat color=cyan no-caps icon=smart_toy").classes("text-caption")
        ...
        with ui.tab_panel(pipeline_tab):
            ...
            ui.separator().classes("q-my-sm")
            from frontend_nicegui.components.morgan_options_ui import render_morgan_options
            render_morgan_options(state)
        ...
        with ui.tab_panel(results_tab):
            ...
            # LLMクイックアクセス: 結果からレポート生成
            ui.separator().classes("q-my-sm")
            with ui.row().classes("items-center justify-end q-mt-sm"):
                ui.button(
                    "🤖 LLMでレポート生成",
                    on_click=lambda: _switch_to_llm_report(),
                ).props("flat color=cyan no-caps icon=description").classes("text-caption")
        ...
        with ui.tab_panel(llm_tab):
            _llm_container = ui.column().classes("full-width")

            def _on_llm_analysis_click():
                """LLM分析ダイアログを開く。"""
                try:
                    ...
                    ui.notify(f"LLM分析ダイアログの起動に失敗: {e}", type="negative")

            def _build_llm():
                _llm_container.clear()
                with _llm_container:
                    # LLM分析ダイアログ起動ボタン
                    with ui.row().classes("items-center q-mb-md"):
                        ui.label("🤖 LLM活用解析").classes("text-h6")
                        ui.button(
                            "💬 LLM対話分析を開く",
                            on_click=lambda: _on_llm_analysis_click(),
                        ).props("color=cyan unelevated no-caps icon=chat").classes("btn-primary")
                    ...
                    with ui.tabs().classes("full-width q-mt-xs").props(
                        ...
                        llm_settings_sub = ui.tab("llm_settings", label="⚙️ LLM設定", icon="settings")
                        llm_report_sub = ui.tab("llm_report", label="📝 レポート生成", icon="description")
                        llm_internal_sub = ui.tab("llm_internal", label="🖥️ 内部AI", icon="memory")

                    with ui.tab_panels(llm_sub_tabs, value=llm_settings_sub).classes("full-width"):
                        with ui.tab_panel(llm_settings_sub):
                            try:
                                from frontend_nicegui.components.llm_settings_ui import render_llm_settings_page
                                render_llm_settings_page()
                            except Exception as e:
                                ui.label(f"LLM設定UIの読み込みに失敗: {e}").classes("text-negative")

                        with ui.tab_panel(llm_report_sub):
                            try:
                                from frontend_nicegui.components.llm_report_tab import render_llm_report_tab
                                render_llm_report_tab(state)
                            except Exception as e:
                                ui.label(f"LLMレポートUIの読み込みに失敗: {e}").classes("text-negative")

                        with ui.tab_panel(llm_internal_sub):
                            try:
                                from frontend_nicegui.components.internal_llm_ui import render_internal_llm_tab
                                render_internal_llm_tab(state)
                            except Exception as e:
                                ui.label(f"内部AI UIの読み込みに失敗: {e}").classes("text-negative")

            _build_llm()
            state["_refresh_llm"] = _build_llm

        def _trigger_llm_from_data_tab():
            """データタブからLLM分析ダイアログを起動する。"""
            df = state.get("df")
            ...
            try:
                ...
                ui.notify(f"LLM分析の起動に失敗: {e}", type="negative")
    ...
    def _switch_to_llm_report():
        """結果タブからLLMタブのレポート生成サブタブへ遷移する。"""
        main_tabs.set_value("llm")
        # レポート生成サブタブを選択（llm_report_subは _build_llm 内で定義されているため、
        # 実際の遷移は _refresh_llm で再描画時に行われる）
        ui.notify("LLMタブでレポートを生成できます", type="info")
    state["_switch_to_llm_report"] = _switch_to_llm_report

    async def _show_llm_advice_dialog():
        """LLMアドバイスを生成してダイアログで表示する。"""
        df = state.get("df")
        if df is None or df.empty:
            ui.notify("先にデータを読み込んでください", type="warning")
            return
        ...
        with ui.dialog() as dialog, ui.card():
            ui.label("🤖 LLMアドバイスを生成中...").classes("text-h6")
            spinner = ui.spinner(size="lg")

            async def generate_and_show():
                try:
                    # 遅延インポートを使用して循環参照を避ける
                    from backend.llm.analysis_advisor import AnalysisAdvisor
                    advisor = AnalysisAdvisor()
                    if not advisor.is_available:
                        advice = advisor._fallback_advice(state)
                    else:
                        advice = advisor.generate_advice(state)
                    # ダイアログをクリアしてアドバイスを表示
                    dialog.clear()
                    with dialog:
                        with ui.card().classes("full-width q-pa-md"):
                            ui.label("🤖 LLMアドバイス").classes("text-h6 q-mb-md")
                            with ui.scroll_area().classes("full-width"):
                                ui.markdown(advice).classes("prose")
                            ui.button("閉じる", on_click=dialog.close).props("flat").classes("q-mt-md")
                except Exception as e:
                    dialog.clear()
                    with dialog:
                        with ui.card().classes("full-width q-pa-md"):
                            ui.label("❌ LLMアドバイスの生成に失敗しました").classes("text-negative text-h6")
    ...
    # データがアップロードされ、目的変数が設定されたらLLM分析を自動起動
    _llm_triggered = {"active": False}

    async def _auto_trigger_llm_analysis():
        """データと目的変数が設定されたらLLM分析を自動起動する。"""
        if _llm_triggered["active"]:
            return  # 既にトリガー済み
        ...
        if df is not None and not df.empty and target_col and target_col in df.columns:
            # LLM分析をトリガー
            _llm_triggered["active"] = True
            try:
                from frontend_nicegui.components.llm_analysis_dialog import trigger_llm_analysis
                trigger_llm_analysis(state, force_new=True)
                ui.notify("🤖 LLMがデータを分析し、解析方針を提案しています", type="info", timeout=3000)
            except Exception as e:
                logger.warning(f"[AutoLLM] LLM分析の自動起動に失敗: {e}")
                _llm_triggered["active"] = False  # エラー時はリセットして再試行可能にする

    # 3秒ごとにチェック。データと目的変数が揃ったらLLM分析を自動起動
    ui.timer(3.0, _auto_trigger_llm_analysis)
...
# ─────────────────────────────────────────────
@app.on_startup
async def _startup_download_check():
    """UI開始後にダウンロードをチェック・実行し、進捗を通知する。"""
    ...
    try:
        ...
        if is_first_launch() or not is_auto_download_complete():
            # ダウンロード開始
            _download_state["active"] = True
            _download_state["progress"] = 0.0
            ui.notify(
                "⏳ 初回起動: Qwen2.5 0.5B をダウンロードしています...",
                type="info",
                timeout=5000,
            )

            def _progress(status: str, frac: float, msg: str) -> None:
                global _download_state
                _download_state["progress"] = frac
                _download_state["message"] = msg
                if status == "done":
                    _download_state["active"] = False
                    _download_state["complete"] = True
                    ui.notify(
                        "✅ Qwen2.5 0.5B の準備が完了しました",
                        type="positive",
                        timeout=5000,
                    )
                elif status == "error":
                    _download_state["active"] = False
                    ui.notify(
                        f"⚠️ Qwen2.5 0.5B のダウンロードに失敗しました: {msg[:100]}",
                        type="warning",
                        timeout=8000,
                    )

            check_and_trigger_first_launch(callback=_progress)
        else:
            _download_state["complete"] = True
            logger.info("[FirstLaunch] Bonsai 8B は既に準備済みです")

    except Exception as _e:
        logger.warning(f"[FirstLaunch] スタートアップチェックに失敗: {_e}")
