#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fix the 目的変数 tab panel and add missing tab panels."""
...
# ============================================================
# Fix 1: Update "目的変数" tab panel to show all descriptors
# ============================================================
# Find the start of the section to replace
import re

# Pattern to find the section: starts with "# ── プロパティをチェックボックスとして表示 ──"
# and ends before "# 全て選択されているかを判定"
pattern1 = r'(# ── プロパティをチェックボックスとして表示 ──.*?)(# 全て選択されているかを判定)'
match1 = re.search(pattern1, content, re.DOTALL)

if match1:
    old_section = match1.group(0)

    # Build the new section
    new_section = '''                                # ── 推奨記述子の表示 ──
...
'''
    ...
    print("Fix 1: Updated 目的変数 tab panel - SUCCESS")
else:
    print("Fix 1: FAILED - pattern not found")

# ============================================================
# Fix 2: Add tab panels for "数えあげ系" and "LLMによる推奨"
# ============================================================
# Find the location after the variance tab panel (after "except ImportError:" for variance)
# and before "# ═══════════════════════════════════════════════════════════"

pattern2 = r'(except Exception as ex:\s+ui\.label\(f"分散計算エラー: \{ex\}"\)\.classes\("text-red"\)\s+)\s+(# ═══════════════════════════════════════════════════════════)'

match2 = re.search(pattern2, content, re.DOTALL)

if match2:
    insert_pos = match2.start(2)  # Position before the comment line
...
'''
    ...
    print("Fix 2: Added tab panels for count and llm - SUCCESS")
else:
